...
async function scrapeIAAI(vin: string): Promise<{ data: IAAIData; html: string }> {
	...
	try {
		...
		console.error('[scrape-iaai] Failed to launch browser:', error);
		throw new Error(
	...
	try {
		...
		try {
			...
			console.log(`[scrape-iaai] Navigation timeout, attempting to scrape partial content`);
		}
		...
		const data = await page.evaluate(() => {
			...
			try {
				...
				const stockNumberEl =
					firstResult.querySelector('[data-uname="stockNumber"]') ||
				...
				// Extract sale date
				const saleDateEl =
					firstResult.querySelector('[data-uname="saleDate"]') ||
				...
				const mileageEl =
					firstResult.querySelector('[data-uname="odometer"]') ||
				...
				const primaryDamageEl =
					firstResult.querySelector('[data-uname="primaryDamage"]') ||
				...
				const secondaryDamageEl =
					firstResult.querySelector('[data-uname="secondaryDamage"]') ||
				...
				// Extract location
				const locationEl =
					firstResult.querySelector('[data-uname="location"]') ||
				...
				// Extract title code
				const titleCodeEl =
					firstResult.querySelector('[data-uname="titleCode"]') ||
				...
				const vehicleTypeEl =
					firstResult.querySelector('[data-uname="vehicleType"]') ||
				...
				const imageElements =
					firstResult.querySelectorAll('img[src*="iaai"]') ||
...
export async function registerScrapeIAAIWorker(queue: PgBoss): Promise<void> {
	...
	console.log(
		'[scrape-iaai] Worker registered with batchSize=2, sequential processing for rate limiting'
	);
}
