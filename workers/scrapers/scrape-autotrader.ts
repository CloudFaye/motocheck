		...
		const data = await page.evaluate(() => {
			...
			try {
				...
				const listingContainer =
					document.querySelector('[data-cmp="inventoryListing"]') ||
				...
				const priceEl =
					listingContainer.querySelector('[data-cmp="pricing"]') ||
				...
				const mileageEl =
					listingContainer.querySelector('[data-cmp="mileage"]') ||
				...
				const dealerEl =
					listingContainer.querySelector('[data-cmp="sellerName"]') ||
				...
				const locationEl =
					listingContainer.querySelector('[data-cmp="sellerLocation"]') ||
				...
				const dateEl =
					listingContainer.querySelector('[data-cmp="listingDate"]') ||
				...
				const yearEl =
					listingContainer.querySelector('[data-cmp="year"]') ||
				...
				const makeEl =
					listingContainer.querySelector('[data-cmp="make"]') ||
				...
				const modelEl =
					listingContainer.querySelector('[data-cmp="model"]') ||
				...
				const trimEl =
					listingContainer.querySelector('[data-cmp="trim"]') ||
				...
				// Extract colors
				const exteriorColorEl =
					listingContainer.querySelector('[data-cmp="exteriorColor"]') ||
					document.querySelector('[class*="exterior-color"]');
				if (exteriorColorEl) {
					result.exteriorColor = exteriorColorEl.textContent?.trim() || null;
				}

				const interiorColorEl =
					listingContainer.querySelector('[data-cmp="interiorColor"]') ||
				...
				const transmissionEl =
					listingContainer.querySelector('[data-cmp="transmission"]') ||
				...
				// Extract drivetrain
				const drivetrainEl =
					listingContainer.querySelector('[data-cmp="drivetrain"]') ||
				...
				// Extract fuel type
				const fuelTypeEl =
					listingContainer.querySelector('[data-cmp="fuelType"]') ||
				...
				// Extract body style
				const bodyStyleEl =
					listingContainer.querySelector('[data-cmp="bodyStyle"]') ||
				...
				const imageElements =
					document.querySelectorAll('img[src*="autotrader"]') ||
...
export async function registerScrapeAutoTraderWorker(queue: PgBoss): Promise<void> {
	await queue.work(Jobs.SCRAPE_AUTOTRADER, handleScrapeAutoTrader);

	console.log(
		'[scrape-autotrader] Worker registered (optional source - pipeline continues on failure)'
	);
}
