	...
	try {
		...
		const data = await page.evaluate(() => {
			...
			try {
				...
				const accidentEl = document.querySelector(
					'[data-field="accidents"], .accident-history, .accidents'
				);
				if (accidentEl) {
				...
				const ownershipEl = document.querySelector(
					'[data-field="owners"], .ownership-history, .owners'
				);
				if (ownershipEl) {
				...
				const imageElements = document.querySelectorAll(
					'img[src*="vininspect"], img.vehicle-image, img[data-type="vehicle"]'
				);
...
export async function registerScrapeVINInspectWorker(queue: PgBoss): Promise<void> {
	...
	console.log(
		'[scrape-vininspect] Worker registered with batchSize=2, sequential processing for rate limiting'
	);
}
