	...
	try {
		...
		const data = await page.evaluate(() => {
			...
			try {
				...
				const secondaryDamageEl = firstResult.querySelector('[data-uname="lotsearchDamageSecondary"]');
...
export async function registerScrapeCopartWorker(queue: PgBoss): Promise<void> {
	...
	console.log('[scrape-copart] Worker registered with batchSize=2, sequential processing for rate limiting');
}
