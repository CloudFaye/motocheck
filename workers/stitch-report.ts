...
async function stitchReportWorker(job: Job<StitchJobPayload>): Promise<void> {
	...
	try {
		...
		console.log(`[stitch-report] Found ${normalizedRecords.length} normalized records for VIN ${vin}`);
		
		// Parse normalized data
		const records: NormalizedVehicleRecord[] = normalizedRecords.map(r => 
			r.data as unknown as NormalizedVehicleRecord
		);
		...
		// Store timeline in reports table
...
async function buildTimeline(
	...
	allEvents.sort((a, b) => {
		...
		const sourceOrder = ['nhtsa_decode', 'nhtsa_recalls', 'nmvtis', 'nicb', 'copart', 'iaai', 'autotrader', 'cargurus'];
 ...
 */
async function detectOdometerAnomalies(
	vin: string,
	...
	for (let i = 0; i < readings.length; i++) {
		...
		if (i > 0) {
			...
			if (reading.mileage < prevReading.mileage) {
				...
				console.log(`[stitch-report] Rollback detected: ${prevReading.mileage} -> ${reading.mileage} miles`);
			}
			...
			if (!isAnomaly) {
				...
				if (yearsDiff > 0) {
					...
					if (annualRate > 50000) {
						...
						console.log(`[stitch-report] Unusual mileage rate detected: ${Math.round(annualRate)} miles/year`);
					}
				}
			}
		}
...
function detectHistoryGaps(events: VehicleEvent[]): HistoryGap[] {
	...
	for (let i = 1; i < sortedEvents.length; i++) {
		...
		if (monthsDiff >= 18) {
			...
			console.log(`[stitch-report] Gap detected: ${Math.round(monthsDiff)} months (${severity} severity)`);
		}
	}
	...
	if (sortedEvents.length > 0) {
		...
		if (monthsSinceLastEvent >= 18) {
			...
			console.log(`[stitch-report] Gap to present detected: ${Math.round(monthsSinceLastEvent)} months (${severity} severity)`);
		}
	}
