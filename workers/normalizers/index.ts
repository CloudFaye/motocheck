 ...
 */
type NormalizerFunction = (vin: string, rawJson: unknown, rawHtml: string | null) => Promise<NormalizedVehicleRecord>;
...
async function checkAndEnqueueStitching(vin: string): Promise<void> {
	...
	const allRequiredComplete = REQUIRED_SOURCES.every(source => 
		completedSources.has(source)
	);
	
	if (allRequiredComplete) {
		console.log(`[normalize] All required sources complete for VIN ${vin}, enqueueing stitching job`);
