 ...
 * - Theft Record status
 * 
 * Each card has an icon (emoji), large number, label, and color-coded badge.
 * 
 * @param data - Complete vehicle data
 * @returns Array of paragraphs forming the highlights section
 */
export function buildKeyHighlightsSection(data: ComprehensiveVehicleData): (Paragraph | Table)[] {
	...
	content.push(
		...
		new Paragraph({
			children: [
				new TextRun({
					text: ownersCount <= 2 ? '✓ Clean' : ownersCount <= 4 ? '⚠ Multiple' : '⚠ Many',
					font: DOCX_STYLES.fonts.body,
					size: 20,
					bold: true,
					color: ownersCount <= 2 ? DOCX_STYLES.colors.success : DOCX_STYLES.colors.warning
				})
			],
			spacing: { after: 200 },
			alignment: AlignmentType.CENTER
	...
	content.push(
		...
		new Paragraph({
			children: [
				new TextRun({
					text: accidentsCount === 0 ? '✓ No Accidents' : accidentsCount === 1 ? '⚠ 1 Accident' : '⚠ Multiple',
					font: DOCX_STYLES.fonts.body,
					size: 20,
					bold: true,
	...
	content.push(
		...
		new Paragraph({
			children: [
				new TextRun({
					text: recallsCount === 0 ? '✓ No Recalls' : '⚠ Check Recalls',
					font: DOCX_STYLES.fonts.body,
					size: 20,
					bold: true,
	...
	content.push(
		...
		new Paragraph({
			children: [
				new TextRun({
					text: isStolen ? 'STOLEN' : 'No',
					font: DOCX_STYLES.fonts.heading,
					size: 48,
					bold: true,
					color: isStolen ? DOCX_STYLES.colors.error : DOCX_STYLES.colors.primary
				})
			],
			spacing: { before: 60, after: 60 },
			alignment: AlignmentType.CENTER
